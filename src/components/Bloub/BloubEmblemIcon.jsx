...
/**
 * Generates an SVG path for a standard upright capsule eye
 * centered precisely at (cx, cy).
 */
const makeCapsuleEyePath = (cx, cy, w = 9.5, h = 21) => {
  const hw = w / 2;
  const hh = h / 2;
  const r = hw;
  return [
    'M', +(cx - hw).toFixed(2), +(cy - hh + r).toFixed(2),
    'A', +(r).toFixed(2), +(r).toFixed(2), '0 0 1', +(cx - hw + r).toFixed(2), +(cy - hh).toFixed(2),
    'L', +(cx + hw - r).toFixed(2), +(cy - hh).toFixed(2),
    'A', +(r).toFixed(2), +(r).toFixed(2), '0 0 1', +(cx + hw).toFixed(2), +(cy - hh + r).toFixed(2),
    'L', +(cx + hw).toFixed(2), +(cy + hh - r).toFixed(2),
    'A', +(r).toFixed(2), +(r).toFixed(2), '0 0 1', +(cx + hw - r).toFixed(2), +(cy + hh).toFixed(2),
    'L', +(cx - hw + r).toFixed(2), +(cy + hh).toFixed(2),
    'A', +(r).toFixed(2), +(r).toFixed(2), '0 0 1', +(cx - hw).toFixed(2), +(cy + hh - r).toFixed(2),
    'Z'
...
/**
 * 3D Gold Metal Emblem Icon for Character Bloub in GameOver & Leaderboard Cards.
...
export const BloubEmblemIcon = ({
  shape = DEFAULT_SHAPE,
  isWinner = false,
  expression = null, // 'victory' | 'normal' | 'defeat'
  className = "w-36 h-36 sm:w-40 sm:h-40"
}) => {
  const shapeObj = SHAPE_BY_ID.get(shape) || SHAPE_BY_ID.get(DEFAULT_SHAPE);
  const radii = shapeObj?.radii || new Array(64).fill(1);

  // Expression resolution: explicit expression takes priority, fallback to isWinner
  const effectiveExp = expression || (isWinner ? 'victory' : 'defeat');
  const isVictory = effectiveExp === 'victory';
  const isNormal = effectiveExp === 'normal';

  // Position and Scale: Victory is shifted slightly down to accommodate the top crown
  const cx = 80;
  const cy = isVictory ? 86 : 80;
  const scale = isVictory ? 43 : 47;
  ...
  // Generate Eye paths based on expression:
  // - Victory: joyful smiling squint
  // - Normal: iconic upright capsule eyes
  // - Defeat: rounded pill crosses
  const leftEyeD = isVictory
    ? makeSmilingEyePath(cx - 16, cy - 3, 19, 9, 4.5)
    : isNormal
      ? makeCapsuleEyePath(cx - 14, cy, 9.5, 21)
      : makeCrossPath(cx - 16, cy, 21, 5.5);

  const rightEyeD = isVictory
    ? makeSmilingEyePath(cx + 16, cy - 3, 19, 9, 4.5)
    : isNormal
      ? makeCapsuleEyePath(cx + 14, cy, 9.5, 21)
      : makeCrossPath(cx + 16, cy, 21, 5.5);
